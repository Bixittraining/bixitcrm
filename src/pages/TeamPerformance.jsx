...
import {
  Users, Target, GraduationCap, UserX, ShieldAlert,
} from 'lucide-react'
...
export default function TeamPerformance() {
  ...
  // fabricated. Everything below comes straight from the leads table.
  const rows = useMemo(() => {
    return teamMembers.map((member) => {
      ...
      return { member, total, notAttempted, contacted, qualified, negotiation, enrolled, lost, conversionRate }
  ...
  }), { total: 0, enrolled: 0, lost: 0 }), [rows])
    ...
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={isDark ? 'bg-dark-800/80' : 'bg-dark-50/80'}>
                {['Agent', 'Role', 'Total Assigned', 'Not Attempted', 'Contacted', 'Qualified', 'Negotiation', 'Enrolled', 'Lost', 'Conversion'].map((h) => (
            ...
            <tbody className={`divide-y ${isDark ? 'divide-dark-800' : 'divide-dark-100'}`}>
              {rows.map(({ member, total, notAttempted, contacted, qualified, negotiation, enrolled, lost, conversionRate }) => (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`rounded-2xl p-4 flex items-start gap-3 ${cardClass}`}>
        ...
        <p className={`text-xs ${isDark ? 'text-dark-400' : 'text-dark-500'}`}>
          This shows real lead volume and conversion per agent, all derived from actual lead assignment and status. Call-level metrics (calls made, missed calls, talk-time) aren't shown because this CRM has no phone/telephony integration to measure them — adding those would mean showing fabricated numbers.
        </p>
      </motion.div>
    </div>
  )
